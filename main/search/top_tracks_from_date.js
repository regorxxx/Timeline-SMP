...
function topTracksFromDate({
	...
	// Sanity checks
	if (!isPlayCount) { fb.ShowPopupMessage('top_tracks_from_date: foo_playcount component is not installed.', window.FullPanelName); return; }
	if (!isEnhPlayCount) { fb.ShowPopupMessage('top_tracks_from_date: foo_enhanced_playcount is not installed.', window.FullPanelName); return; }
...
function getSkipCount(handleList, timePeriod, timeKey = null, fromDate = new Date()) {
	if (!isSkipCount) { fb.ShowPopupMessage('getSkipCount: foo_skipcount is not installed.', window.FullPanelName); return []; }
